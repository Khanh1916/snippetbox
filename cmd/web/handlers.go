...
func (app *application) snippetCreatePost(w http.ResponseWriter, r *http.Request) {
	...
	form.CheckField(validator.PermittedValue(form.Expires, 1, 7, 365), "expires", "This field must euqal 1, 7, 365.")
...
func (app *application) userLoginPost(w http.ResponseWriter, r *http.Request) {
	...
	err = app.sessionManager.RenewToken(r.Context()) // create new token for user session
	if err != nil {
		app.serverError(w, err)
		return
	}
...
func (app *application) accountPasswordUpdatePost(w http.ResponseWriter, r *http.Request) {
	...
	if !form.Valid() {
		data := app.newTemplateData(r)
		data.Form = form
		// if false render the page again with red message
	...
	if err != nil {
		if errors.Is(err, models.ErrInvalidCredentials) {
			...
			data := app.newTemplateData(r)
			data.Form = form
			// if current password be incorrect render again
			app.render(w, http.StatusUnprocessableEntity, "password.html", data)
		} else {
			app.serverError(w, err)
		}
